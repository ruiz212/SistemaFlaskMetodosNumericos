def secante(x0, x1, tol, f):
    resultados = []
    c_prev, c_actual, iteracion = x0, x1, 1
    ...
    while True:
        ...
        error_rp = abs((c_nuevo - c_actual) / c_nuevo) * 100.0 if (iteracion > 1 and c_nuevo != 0) else 100.0
        error_str = f"{error_rp:.4f}%" if iteracion > 1 else "---"
        
        resultados.append({
            'iter': iteracion, 'ci': f"{c_nuevo:.7f}", 'fci': f"{f_nuevo:.7f}", 'error': error_str
        })
        
        if (iteracion > 1 and error_rp < tol) or f_nuevo == 0:
